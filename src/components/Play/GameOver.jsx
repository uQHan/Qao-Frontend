...
export default function GameOver() {
	const { queries, score, win, questions, setDecryptedAnswer, takeId } = useBoundStore(state => state)
	...
	return (
		<>
			...
			<dialog id="gameoverdialog" open={true} className="fixed m-0 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4/5 max-w-4xl px-6 py-12 rounded-md bg-white text-slate-900 z-40">
				<button className="absolute top-2 right-2 text-3xl hover:scale-105" onClick={closeDialog}>
					<IoCloseSharp />
				</button>

				{/* Flex container for the main panel and the list */}
				<div className="flex flex-col md:flex-row gap-6 items-center">
					...
					<div className="flex flex-col items-center gap-4 flex-1">
						...
						<div className="flex gap-6 items-center">
							...
							<button
								...
								className="btn-primary px-5 md:px-10 py-3 uppercase tracking-widest rounded-md bg-blue-500 text-white"
							>
					...
					<div className="flex-1 w-full md:w-1/2">
						<ul className="w-full max-h-96 overflow-y-auto bg-gray-100 p-4 rounded-md">
							{questions.map((question, index) => (
								...
								>
									{/* Question Text */}
									{/* <p className={`text-sm font-bold ${question.userAnswer > 0
															? ' text-green-600 '
															: ' text-red-600'
														} text-center`}>
											...
											>
												<div className="flex flex-col gap-2">
													...
													<button
														...
														onClick={() => alert('Ask AI functionality coming soon!')}
													>
														Ask AI
													</button>
												</div>
